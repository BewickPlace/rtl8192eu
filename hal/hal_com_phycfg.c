...
)
{
	...
	{
		rtw_merge_string(ffile_path, PATH_LENGTH_MAX, rtw_phy_file_path, pFileName);
	
		if (rtw_is_file_readable(ffile_path) == _TRUE)
		{
			rlen = rtw_retrive_from_file(ffile_path, pHalData->para_file_buf, MAX_PARA_FILE_BUF_LEN);
			if (rlen > 0)
			{
				rtStatus = _SUCCESS;
